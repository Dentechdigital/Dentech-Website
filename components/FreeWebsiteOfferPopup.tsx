...
export default function FreeWebsiteOfferPopup() {
  ...
  return (
    <dialog
      ref={dialogRef}
      className="fixed inset-0 z-[80] max-h-[min(92dvh,720px)] w-[calc(100%-1.5rem)] max-w-lg translate-y-0 overflow-hidden rounded-2xl border border-slate-200/90 bg-white p-0 text-slate-800 shadow-2xl backdrop:bg-slate-950/50 open:flex open:flex-col dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 sm:max-w-xl"
      aria-labelledby="free-offer-popup-title"
      aria-describedby="free-offer-popup-desc"
      onClose={close}
    >
      <div className="flex max-h-[inherit] flex-col">
        <div className="flex items-start justify-between gap-3 border-b border-slate-100 px-4 py-4 sm:px-6 dark:border-slate-800">
          <div className="min-w-0 pr-2">
            <p className="text-xs font-semibold uppercase tracking-wide text-blue-600 dark:text-blue-300">New practice offer</p>
            <h2 id="free-offer-popup-title" className="mt-1 font-serif text-xl font-bold tracking-tight text-blue-950 dark:text-white sm:text-2xl">
              Website included with your growth plan
            </h2>
          </div>
          <button
            type="button"
            className="shrink-0 rounded-full p-2 text-slate-500 transition hover:bg-slate-100 hover:text-slate-800 dark:hover:bg-slate-800 dark:hover:text-white"
            aria-label="Close"
            onClick={close}
          >
            <X className="h-5 w-5" />
            ...
            <form className="mt-6 space-y-3" onSubmit={handleSubmit} noValidate>
              <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                ...
                <input
                  ...
                  className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 outline-none ring-blue-500/30 focus:ring-2 dark:border-slate-600 dark:bg-slate-950 dark:text-white"
                  required
                />
              </label>
              <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
              ...
              <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                ...
                <textarea
                  ...
                  className="mt-1 w-full resize-none rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 outline-none ring-blue-500/30 focus:ring-2 dark:border-slate-600 dark:bg-slate-950 dark:text-white"
                />
              </label>
